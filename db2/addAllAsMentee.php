	...
	if (isset($_GET['sid']) && isset($_SESSION['user'])) {
		...
		if ($current_id == $sid || ($_SESSION['isParent'] && $in_all_childen_of_parent) || $_SESSION['isAdmin']) {
			...
			if ($mentee_count < 6) {
				...
				$possible_meetings_mentee_of = "SELECT meet_id FROM meetings WHERE group_id = '$userPage_group_id' AND meet_id NOT IN (SELECT meet_id FROM enroll WHERE mentee_id = '$sid')";
